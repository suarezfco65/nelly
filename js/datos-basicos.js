const datosBasicos = {
  ...
  async cargarDatos() {
    try {
      ...
      try {
        const responseEncriptado = await fetch(
          "json/datos-basicos-encriptado.json"
        );
        ...
        console.log(
          "No hay datos encriptados o clave incorrecta:",
          errorEncriptado
        );
      }
  ...
  async manejarModificacion(event) {
    ...
    try {
      ...
      // Guardar en GitHub usando el token actual (VERSIÓN SIN ENCRIPTACIÓN)
      const result = await this.guardarEnGitHub(
        datosModificados,
        this.tokenActual
      );
